    ...
    return(
        <main className="max-w-6xl py-12 mx-auto">
            <div className="flex flex-col gap-2 justify-center items-center">
                <span className="text-4xl font-semibold">Choose Your Plan</span>
                <h1 className="text-lg text-center w-2xl">
                ...
                {plans.map((item, index) => (
                    <div className="" key={index}>
                        ...
                        <div className="relative p-0.5 bg-linear-to-br from-neutral-200 via-neutral-300 to-neutral-200 rounded-2xl hover:scale-105 duration-300">
                            <div className="flex flex-col gap-2 items-center justify-center w-xs bg-neutral-50 rounded-2xl">
                            ...
                            <ul className="text-sm flex flex-col items-start pb-4">
                                {item.includes.map((item, index) => (
                                    <li key={index} className="flex gap-2 items-center justify-center">
                                        <FaCheck />{item}
                                    </li>
                                ))}
                            </ul>
                        ...
                        : 
                        <div className="flex flex-col gap-2 bg-white rounded-2xl border border-neutral-300 hover:scale-105 duration-300 items-center justify-center w-xs">
                            <span className="text-xs bg-emerald-400 mt-8 px-6 uppercase py-2 rounded-full border">{item.planType}</span>
                            <div className="flex flex-col px-6 py-2">
                                <span className="line-through text-sm">{item.strikeThrough}</span>
                                <span className="text-4xl">{item.pricing}
